...
def main(): 
    ...
    lr = config['CFCP']['lr']
    forward_loss_ratio = 1# config['CFCP'].get('forward_loss_ratio', 1.0)  # Default to 1.0 if not specified
    # -----------------------
    ...
    for _ in range(epochs):
        ...
        if (_ + 1) > 10000 and end_loss > loss_forward.item():
            ...
            torch.save(realnvp_model.state_dict(), os.path.join(save_path, f"CFCPmodel_{num_nodes}_nd.pth"))
            ...
            fig.tight_layout()
            fig.savefig(os.path.join(save_path, f"CFCP_gen_nd.png"))
