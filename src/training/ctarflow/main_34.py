...
def main(): 
    ...
    num_layers = config['CTarflow']['num_layers']
    input_dim = config['CTarflow']['input_dim']
    num_blocks_encoder = config['CTarflow']['num_blocks_encoder']
    output_dim = config['CTarflow']['output_dim']
    embed_dim = config['CTarflow']['embed_dim']
    num_heads = config['CTarflow']['num_heads']
    num_nodes_model = config['CTarflow']['num_nodes']
    ...
    forward_loss_ratio = config['CTarflow']['forward_loss_ratio']
    ...
    for _ in range(epochs):
        ...
        if (_ + 1) > 1000 and end_loss > loss_vtp.item():
            ...
            torch.save(model.state_dict(), os.path.join(save_path, f"Tarflowmodel_{num_nodes}.pth"))
            ...
            fig.tight_layout()
            fig.savefig(os.path.join(save_path, f"Tarflow_gen.png"))

            # ✅ log the figure object, not `plt`
            # wb.log({"Tarflow_gen": fig})
