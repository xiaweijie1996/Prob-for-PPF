...
def main(): 
    ...
    forward_loss_ratio = 0.95 # config['MixedGraphAttention'].get('forward_loss_ratio', 1.0)  # Default to 1.0 if not specified
    ...
    edge_index -= 1  # Convert to zero-based index [0, 32], 33 nodes 
    
    # Convert edge_index to adjacency matrix
    adj_info = torch.zeros((num_nodes-1, num_nodes-1)).to(device)
    adj_info[edge_index[0], edge_index[1]] = 1
    adj_info[edge_index[1], edge_index[0]] = 1  # undirected graph (num_nodes, num_nodes)
   
    # Add self-loops
    adj_info += torch.eye(num_nodes-1).to(device)
    
    # Add one row and one column for the position token
    graph_info = torch.ones(adj_info.shape[0]+2, adj_info.shape[1]+2).to(device)
    graph_info[:-2, :-2] = adj_info
    ...
    wb.init(project=f"MixedAttention-graph-node-{num_nodes}")
    
    # Log Model size
    wb.log({"Model Parameters": sum(p.numel() for p in mix_model.parameters() if p.requires_grad)})
    
    # Load already trained model if exists
    model_path = os.path.join(save_path, f"MixedAttentionmodel_graph_{num_nodes}.pth")
    ...
    for _ in range(epochs):
        ...
        if (_ + 1) > 1000 and end_loss > loss_vtp.item():
            ...
            torch.save(mix_model.state_dict(), os.path.join(save_path, f"MixedAttentionmodel_{num_nodes}_higherratio.pth"))
            ...
            fig.tight_layout()
            fig.savefig(os.path.join(save_path, f"MixedAttention_gen_higherratio.png"))
