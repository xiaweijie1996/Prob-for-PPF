...
batch_size = config['CRealnvp']['batch_size'] * 50  # Use a larger batch size for evaluation
...
v_index = p_index
n_bins = 40  # Number of bins for the pdf and cdf plot
...
model.eval()
with torch.no_grad():
    pre_v, _jaf = model.forward(input_x, input_c, index_p=p_index, index_v=v_index)
    pre_p, _jai = model.inverse(output_y, input_c, index_p=p_index, index_v=v_index)
...
plt.legend()
# plt.tight_layout()
plt.savefig(f'figures/CRealnvp_{num_nodes}node_accuracy1.png')
...
plt.legend()
# plt.tight_layout()
plt.savefig(f'figures/CRealnvp_{num_nodes}node_accuracy2.png')
plt.close()
# -----------------------
